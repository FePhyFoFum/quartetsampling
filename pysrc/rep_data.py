...
def chi2_test(val0, val1):
    """Calculate Pearson Chi-Squared for the special case of
       ...
       Arguments:
           ...
           val1: second value
    """
    # <JGLAHE> potential DLL issues w/scipy (possibly check/to docs)
...
def process_replicate_raxmlng_lrt(replicate):
    ...
    result = {}
    result["label"] = replicate['unique_label']
    # <JGLAHE/12.i.23> 'reduced' (temp) alignment file extensions modified to
    # <JGLAHE/12.i.23> match that of the file generated by raxml-ng.
    temp_file_paths = [replicate['aln_fname'],
    ...
    lpath = os.path.join(replicate['temp_wd'],
                         "{}.raxml.log".format(replicate['aln_fname']))
    # <JGLAHE> significance of tpath in functions where lnlike threshold > 0?
    tpath = os.path.join(replicate['temp_wd'],
                         "{}.raxml.bestTree".format(replicate['aln_fname']))
    temp_file_paths.extend([lpath, tpath])
    # <JGLAHE/12.i.23> Added additional raxml-ng output file extensions so that
    # <JGLAHE/12.i.23> temp_file_paths captures everything raxml-ng generates.
